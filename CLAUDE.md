...
## 規模（2026-09-14 時点の実測値）

| 項目 | 数 |
|---|---|
| 図鑑の登録ポケモン | 68 |
| うちメガシンカできる個体 | 16（メガ形態を別に数えると84） |
| 技 | 191（変化54 / 物理72 / 特殊65） |
| 持ち物 | 73（メガストーン17 / その他56） |
| 図鑑で使われている特性の種類 | 111 |
| カスタム作成で選べる特性（IMPL_ABILITIES） | 113 |

未実装のまま図鑑に載っている特性が4つある（きけんよち・ほうし・マイペース・
のろわれボディ）。ゲンガーはのろわれボディしか持たないため実質特性なしになる。

数え直すときはハーネスで `DEX.length` / `Object.keys(MOVES).length` /
`Object.keys(ITEMS).length` を出す。手で数えない。
